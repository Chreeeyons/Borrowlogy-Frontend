        ...
        <>
          ...
          <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[#83191c] shadow-inner md:hidden">
            <ul className="flex justify-around items-center py-2 text-white text-xs font-semibold font-['Jost']">
              {[
                { href: "/equipment", icon: <GiMicroscope className="text-xl" />, label: "LAB" },
                { href: "/chemicals", icon: <FlaskConical className="text-xl" />, label: "CHEM" },
                { href: "/cart", icon: <FiShoppingCart className="text-xl" />, label: "CART" },
                { href: "/history", icon: <FiClock className="text-xl" />, label: "HISTORY" },
              ].map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`flex flex-col items-center justify-center px-3 py-1 transition-colors duration-200 ease-in-out
                      ${pathname === item.href ? "underline underline-offset-4" : "hover:underline hover:underline-offset-4"}
                    `}
                  >
                    {item.icon}
