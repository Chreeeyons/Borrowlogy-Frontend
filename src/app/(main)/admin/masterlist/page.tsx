...
import { addUser, getUsers, updateHistory } from "@/services/userService";
...
const Equipments = () => {
  ...
  const getUsersData = async () => {
    ...
    if (response && response.custom_users) {
      ...
      console.log(response.custom_users);
    }
  };
  ...
  const handleCSVUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: async function (results: Papa.ParseResult<any>) {
        ...
        const validUsers = parsedData.filter(
          (user) =>
  ...
  const [lab_remarks, setLabRemarks] = useState("");
  const [transactionId, setTransactionId] = useState<number | null>(null);
  const [cartId, setCartId] = useState<number | null>(null);

  const handleReturnBorrowedItem = async () => {
    await updateHistory({
      items: items,
      remarks: remarks,
  ...
  useEffect(() => {
    if (selectedTransaction) {
      ...
      setCartId(selectedTransaction.cart);
      setItems(selectedTransaction.cart_items || []);
      setRemarks(selectedTransaction.remarks || "");
      setLabRemarks(selectedTransaction.labtech_remarks || "");
    }
  }, [selectedTransaction]);
      ...
      <div className="p-6 text-black">
        <div className="bg-[#EEE9E5] rounded-[20px] p-6 shadow-[inset_3px_3px_6px_0px_rgba(0,0,0,0.25)]">
          ...
          <table className="w-full mb-6 text-black table-auto">
            ...
            <tbody>
              {selectedTransaction.cart_items?.map((item: any, index: any) => (
                <tr key={index}>
                  <td className="px-4 py-2">{item.equipment_name}</td>
                  <td className="px-4 py-2">{item.total_quantity} pcs</td>
          ...
          <div className="flex gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <textarea
                value={selectedTransaction.labtech_remarks || ""}
                onChange={(e) =>
          ...
          <button
            className={
              selectedTransaction.status == "Returned" ? "hidden" : `block`
            }
            onClick={() => {
              handleReturnBorrowedItem();
              setSelectedTransaction(null);
            }}
            style={{
              width: "130px",
      ...
      <div className="p-6 text-black">
        <div className="bg-[#EEE9E5] rounded-[20px] p-6 shadow-[inset_3px_3px_6px_0px_rgba(0,0,0,0.25)]">
          ...
          <table className="w-full mb-4 text-black table-auto">
            ...
            <tbody>
              {selectedBorrower.transactions.map((tx: any, index) => (
                <tr
                  key={index}
                ...
                >
                  <td className="px-4 py-2 text-center">{tx.id}</td>
                  {""}
                  {/* Centered text */}
                  ...
                  <td
                    className={`px-4 py-2 text-center ${statusColor(
                      tx.status == "Returned" ? "RETURNED" : "PENDING"
                    )}`}
                    style={{
